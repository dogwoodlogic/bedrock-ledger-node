...
api.consensus._hasher = (data, callback) => {
  lambdaNormalize.Payload = JSON.stringify(data);
  // normalize ledger event to nquads
  lambda.invoke(lambdaNormalize, (err, result) => {
    if(err) {
      return callback(err);
    }
    callback(null, niUri.digest('sha-256', JSON.parse(result.Payload), true));
  });
};
