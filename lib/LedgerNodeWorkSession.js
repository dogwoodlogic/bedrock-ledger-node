...
let ABORT_WORK_SESSIONS;
bedrock.events.on('bedrock.exit', () => {
  logger.info(`Ledger work session terminating...`);
  ABORT_WORK_SESSIONS = true;
});
...
module.exports = class LedgerNodeWorkSession {
  ...
  timeRemaining() {
    return !ABORT_WORK_SESSIONS ?
      Math.max(0, this.startTime + this.maxAge - Date.now()) :
      0;
  }
  ...
  async start(maxAge, fn) {
    ...
    try {
      ...
      logger.verbose(
        `Error reserving work session (${this.id})`, {
          error: e,
          sessionId: this.id,
          ledgerNodeId: this.ledgerNode.id
        });
